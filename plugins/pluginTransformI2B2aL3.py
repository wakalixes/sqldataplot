...
class PluginTransformI2B2aL3(PluginTransform):
    def __init__(self):
    ...
    def transform(self,data, axes, constants):
      ...
      def l3effective(natoms):
          return (8.0*np.sqrt(3.0)*(kb**3)*((NAT**3)-(natoms**3))*(np.pi**3)*(TEMP**3))/((mass**3)*(NAT**2)*(natoms**3)*TIME*(WM**6))
 ...
 #(NN'[t] == -L3Th[B] n3ave[T, wm] *  NN[t]^3)
      ...
      NAT = constants[7]#100000
      TIME = constants[8]#0.5
      
      Bdata = (data[0,:] - A0)*COILCONVERSION + FESHBACHRES_CENTER
      adata = map(convB2a, Bdata) # stores scattering length
      data[0,:] = adata
      data[1,:] = np.array(map(l3effective,data[1,:]))
      return data

    def getConstants(self):
        return ["A0", "Coil-Conversion", "Feshbach-resonance center", "Temp", "wx","wy","wz","N0","Waiting-Time"]
